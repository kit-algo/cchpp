...
g = sns.lineplot(data=queries.query('graph == "Europe" & ball_size_exp in [14,17,21,24] & hostname == "compute11"'), x='terminal_idx', y='passed_time_ms', hue='Ball Size $|B|$', style='Algorithm', ci=None, marker='X', ax=ax1)
g.set_xlabel('Number of Computed Distances')
g.set_ylabel('Running Time [ms]')

g = sns.lineplot(data=queries.query('graph == "Europe" & ball_size_exp in [14,17,21,24] & hostname == "compute11"'), x='terminal_idx', y='single_query_time_mus', hue='Ball Size $|B|$', style='Algorithm', ci=None, marker='X', ax=ax2, legend=False)
g.set_yscale('log')
g.set_xlabel('Number of Computed Distances')
...
fig.tight_layout()
fig.savefig('paper/fig/lazy_rphast_et_vs_dfs.pdf')

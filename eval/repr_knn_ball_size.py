...
data = pd.read_csv(valentin_base + f"SEA21/POIs/VaryingBallSize_CCH.index.csv")
data['phase'] = 'select'
data['family'] = 'CCH [orig]'
data['algo'] = 'cch_orig_select'
times.append(data)
data = pd.read_csv(valentin_base + f"SEA21/POIs/VaryingBallSize_CCH.query.csv")
data['phase'] = 'query'
data['family'] = 'CCH [orig]'
...
plot = sns.lineplot(data=combined.query('num_targets_to_find == 4 & target_set_size_exp == 14'), x='ball_size_exp', y='running_time_ms', hue='family', style='phase', dashes={'select': '', 'query': ''}, markers={'select': 'o', 'query': 'X'}, ci=None)
plot.set_yscale('log')
plot.set_xlabel('Ball Size $|B|$')
plot.set_ylabel('Running Time [ms]')
plot.set_ylim(0.004137743680123092, 12220.431221367462)
...
fig.tight_layout()
fig.savefig('paper/fig/repr_knn_ball_size.pdf')

...
The algorithm iterates over all vertices by ascending rank.
...
This decomposition is very useful for several CCH algorithms, for example for the parallelization of the customization or k-nearest-neighbor queries.
However, some blackbox partitioners such as FlowCutter or InertialFlowCutter will only return the vertex order and not the separator decomposition.
Luckily, we can reconstruct a decomposition from the elimination tree.
...
\subsubsection{Faster Triangle Enumeration}

We accelerate the triangle enumeration by introducing an additional array $\mathtt{W}$ of size $n$.
The entry $\mathtt{W}[v]$ will temporarily store the weight of an edge $uv$ for different vertices $u$ throughout the customization.
This allows us to avoid any coordinated sweeps and instead iterate over the edges of two vertices of a triangle and access the weight of the third edge directly in $\mathcal{O}(1)$.
...
\SetKwFunction{Basic}{CustomizeBasic}
\SetKwProg{Fn}{Function}{:}{}
\Fn{\Basic}{
  \For{all vertices $u \in V$ ordered by ascending rank}{
    \For{all edges $uv \in \echu$}{
      $\mathtt{W}[v] \gets \ell^+(uv)$\;
    }
    \For{all edges $uw \in \echd$}{
      \For{all edges $wv \in \echu$}{
        $\mathtt{W}[v] \gets \min(\mathtt{W}[v], \ell^+(uw) + \ell^+(wv))$\;
      }
    }
    \For{all edges $uv \in \echu$}{
      $\ell^+(uv) \gets \mathtt{W}[v]$\;
    }
  }
}
\caption{TODO.}
\label{algo:faster_customization}
\end{algorithm2e}

\subsubsection{Separator-Based Parallelization}
\subsubsection{Parallelized Reduced Graph Construction}
